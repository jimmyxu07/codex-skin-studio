...
const skins: Skin[] = [
  ...
  },
  {
    ...
    hook: 'A restrained generic arena workspace recipe with floodlights, scoreboard-like blocks, and no real-world marks.',
    palette: ['#F8FAFC', '#E7EEF7', '#101827', '#1D4ED8', '#D97706'],
    accent: '#1D4ED8',
    accentAlt: '#F59E0B',
  ...
  },
  {
    ...
    rightsNote: 'No league, team, player, sponsor, or arena-brand assets.',
  },
  {
    ...
    rightsNote: 'No recognizable mecha franchise silhouettes, marks, names, or protected color layouts.'
  },
  {
  ...
  return (
    <div className={`themed-workspace themed-workspace--${size} ${skin.preview ? 'themed-workspace--image' : ''}`} style={style} aria-label={`${skin.name} Codex-like workspace preview`}>
      ...
      <div className="themed-shell">
        ...
        <main><div className="mini-tab">.dreamskin data recipe <i></i></div><div className="mini-cards">{['Home', 'Task', 'Diff', 'Composer'].map((item) => <span key={item}>{item}</span>)}</div><div className="mini-composer">review readability and export… <button type="button">Remix</button></div></main>
      </div>
    </div>
  )
}
...
function Home() {
  const lead = skins[0]
  return <main><section className="hero section-pad"><div className="hero-copy"><p className="eyebrow">CODEX DREAM SKIN BUILDER · LOCAL · DATA-ONLY</p><h1>Build your own Codex Dream Skin.</h1><p className="lede">Anime-style, idol-stage, sports-arena, fantasy, esports and personal-photo workspaces — start from original builder concepts or upload images you have rights to use.</p><p className="lede">Preview Home / Task / Diff / Composer surfaces, tune crop, focal point, dimming, blur, panel opacity and accent colors, then export a reviewable `.dreamskin` data recipe.</p><div className="home-category-chips"><a href="/skins">Anime</a><a href="/skins/idol-stage">Idol</a><a href="/skins/football-nation-arena">Sports recipe</a><a href="/builder">Upload your image</a><a href="/security">Data-only boundary</a></div><div className="cta-row"><Button href="/builder">Open Builder</Button><Button href="/skins" secondary>Browse Concepts</Button><Button href="/docs/install" secondary>Review apply guide</Button></div><div className="trust-badges"><span>Local image processing</span><span>Data-only recipe export</span><span>Review-first helper path</span><span>Restore path</span><span>Rights-cleared public templates</span></div></div><div className="hero-preview"><WorkspacePreview skin={lead} size="hero" /></div></section><section className="section-pad"><div className="section-heading"><p className="eyebrow">Builder concepts</p><h2>Original preview recipes for remixing</h2><p>These are builder concepts, not finished skin packs. Public concepts are genre-inspired and IP-safe: no real celebrities, characters, teams, flags, crests, tournaments, or brand marks.</p></div><div className="gallery-grid">{skins.slice(0, 3).map((skin) => <SkinCard key={skin.slug} skin={skin} />)}</div></section></main>
}

function Skins() {
  return <main className="section-pad page"><p className="eyebrow">Builder concept gallery</p><h1>Browse Codex Dream Skin preview recipes</h1><p className="page-lede">Emergency public gallery cleanup: entries are builder concepts and remixable preview recipes, not finished skin packs. Use the Builder to export an inert `.dreamskin` data recipe, then review any separate helper before applying.</p><div className="gallery-grid">{skins.map((skin) => <SkinCard key={skin.slug} skin={skin} />)}</div></main>
}

function SkinDetail({ skin }: { skin: Skin }) {
  return <main className="section-pad page"><p className="eyebrow">{skin.category}</p><h1>{skin.name}</h1><p className="page-lede">{skin.hook}</p><section className="work-hero"><div className="work-copy"><h2>Remix this style with a reviewable boundary</h2><p>{skin.rightZone}</p><dl className="metadata-panel"><div><dt>Export</dt><dd>data-only .dreamskin recipe</dd></div><div><dt>Status</dt><dd>{skin.status}</dd></div><div><dt>Rights</dt><dd>{skin.rightsNote}</dd></div><div><dt>Windows</dt><dd>Documented path only; not tested by CodexSkin.fun.</dd></div></dl><div className="cta-row"><Button href={`/builder?preset=${skin.slug}`}>Remix this style</Button><Button href="/docs/install" secondary>Apply/restore guide</Button><Button href="/security" secondary>Security boundary</Button></div></div><WorkspacePreview skin={skin} size="detail" /></section><section className="studio-card adapt-steps"><h2>What exports in the data-only `.dreamskin` recipe</h2><ol><li><code>manifest.json</code> with package type, rights, platform claim split, and unsigned local-export status.</li><li><code>theme.json</code> with focusX/Y, crop, dimming, blur, panel opacity, accent tokens, readability mode, and state preview notes.</li><li><code>background</code> and <code>preview</code> data only — no JS, shell, PowerShell, executable, app bundle, API key, or provider config.</li><li><code>LICENSE.txt</code>, <code>NOTICE.md</code>, <code>user-assets-notice.txt</code> if user uploads assets, <code>sha256.txt</code>, and <code>signature.json</code> marked unsigned.</li></ol></section></main>
}
...
function Builder() {
  ...
  const exportZip = async () => {
    ...
    const files = [
      ...
      { name: 'LICENSE.txt', data: encoder.encode('CodexSkin.fun Builder v0 data recipe. Public template components are original genre-inspired assets. User uploads remain the user\'s responsibility.\n') },
      { name: 'NOTICE.md', data: encoder.encode(`# ${selected.name}\n\nThis .dreamskin data recipe is inert data only. It contains no JavaScript, shell, PowerShell, executable, app bundle, API key, provider config, auto-updater, or installer. Engine/helper is a separate reviewed trust layer.\n\n${selected.rightsNote}\n`) },
...
function App() {
  ...
  setMeta('CodexSkin.fun — Build your own Codex Dream Skin', 'Build anime-style, idol-stage, sports-arena, fantasy, esports and personal-photo Codex-style workspace theme recipes in the browser, then export a data-only .dreamskin artifact.')
  let body = <Home />
